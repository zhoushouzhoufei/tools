...
def parse_args():
    ...
    parser.add_argument("--input-dir", default="data/uavid/uavid_val")
    parser.add_argument("--output-img-dir", default="data/uavid/uavid_val/images")
    parser.add_argument("--output-mask-dir", default="data/uavid/uavid_val/masks")
                ...
                for x in range(0, img.shape[1], stride[1]):
                    ...
                    if img_tile.shape[0] == split_size[0] and img_tile.shape[1] == split_size[1] \
                        ...
                        if mode == 'train':
                            ...
                            # 添加：保存彩色可视化 mask
                            colored_mask = label2rgb(mask_tile)
                            out_colored_mask_path = os.path.join(masks_output_dir,
                                                                 "{}_{}_{}_{}_color.png".format(seq, id, m, k))
                            cv2.imwrite(out_colored_mask_path,
                                        cv2.cvtColor(colored_mask, cv2.COLOR_RGB2BGR))  # 注意颜色空间转换
